...
then
    ...
    CMD="/opt/ventura/framework/scripts/start-generator-qx.sh $PARAMS"
    pdsh -w im-generator-0[1-$NUM_GENERATORS] "nohup /bin/sh -c '$CMD' > /dev/null 2>&1 &"

else
    $EXPERIMENT_DIR/flink-vanilla/bin/flink run -s $RESTART -p $NEW_PARALLELISM $EXPERIMENT_JOBS_DIR/im-job-vanilla-benchmarks_2.11-0.1-SNAPSHOT.jar -qx -sessionDuration $SESSION_DURATION -bidsToGenerate $SIZE_BIDS -bidsRate $DESIRED_BIDS -latencyTrackingInterval 0 -numOfVirtualNodes $VIRTUAL_NODES -sourceParallelism $SOURCE_PARALLELISM -windowType $WINDOW_TYPE -windowSlide $WINDOW_SLIDE -maxParallelism $MAX_PARALLELISM -kafkaServers $KAFKA_SERVERS  -numOfReplicaSlotsHint $REPLICA_SLOTS -windowParallelism $NEW_PARALLELISM -concurrentCheckpoints $PARALLEL_CHECKPOINT -sinkParallelism $NEW_PARALLELISM -checkpointingInterval $CHECKPOINT_INTERVAL -personStreamSizeGb $SIZE_PERSONS -auctionStreamSizeGb $SIZE_AUCTIONS -checkpointingTimeout 50000000 -windowDuration $WINDOW_SIZE -personToGenerate $SIZE_PERSONS -auctionsToGenerate $SIZE_AUCTIONS -personRate $DESIRED_PERSONS -auctionRate $DESIRED_AUCTIONS  > run-80.log 2>&1 & 
fi