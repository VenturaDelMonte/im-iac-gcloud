...
SPECIAL_OPTS="-Dgenerator.bid.ratio=4 -Dgenerator.auction.ratio=0 -Dgenerator.person.ratio=0 -Dgenerator.auction.hot=85"
JAVA_OPTS="-Xms24G -XX:+TieredCompilation -XX:+UseCompressedOops -server -XX:+UseG1GC -Xmx24G $SPECIAL_OPTS"
java $JAVA_OPTS -jar /opt/ventura/framework/data-generator/im-generator-4.0.0-SNAPSHOT.jar -kafkaServers $KAFKA_SERVERS -hostname $(hostname) -personsPartition $NUM_PARTITION_PERSON -auctionsPartition $NUM_PARTITION_AUCTIONS -personsWorkers $NUM_WORKERS_PERSONS -auctionsWorkers $NUM_WORKERS_AUCTIONS -inputSizeItemsAuctions $SIZE_AUCTIONS -inputSizeItemsPersons $SIZE_PERSONS -desiredPersonsThroughputKBSec $DESIRED_PERSONS -desiredAuctionsThroughputKBSec $DESIRED_AUCTIONS -csv /opt/ventura/framework/data-generator/  > /opt/ventura/framework/data-generator/generator-$(hostname).log 2>&1 &
pid=$!
